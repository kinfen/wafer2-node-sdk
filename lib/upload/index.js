...
module.exports = (req) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        // })
        //TODO: req估时受到平台的参数污染，目前无法使用 multiparty 进行文件上传；以后修复
        resolve({
            files:req.files,
        });
    }).then(({files}) => {
        ...
        .then(() => {
            return new Promise((resolve, reject) => {
                // 上传图片, fix the bug where bucket now need xxxx-appid format
                params.Bucket = `${config.cos.fileBucket}-${config.qcloudAppId}`;
                cos.putObject(params, (err, data) => {
                    ...
                    // http://wximg-1252574147.cos.ap-guangzhou.myqcloud.com/1521889929448-HJ-qf2mcM.png
                    resolve({
